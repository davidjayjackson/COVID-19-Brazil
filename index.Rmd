...
daily_new_deaths <-ggplot(US) + 
  ...
  geom_line(aes(x=date,y=MAC,col="Mov. Average"),lwd=1) +
   geom_line(aes(x=date,y=new_cases,col="Daily Deaths")) +
  geom_smooth(aes(x=date,y=new_cases),method = "glm")
...
```
```{r}
ggplot(Weekly_new_cases) + geom_area(aes(x=Weekly,y=new_cases),alpha = 0.5,fill="red") + 
  labs(title="Weekly new_cases",x="Date date", y="Weekly Cases") +
    scale_y_continuous(labels = comma)

ggplot(Weekly_new_deaths) + geom_area(aes(x=Weekly,y=new_deaths),alpha = 0.5,fill="blue") + 
